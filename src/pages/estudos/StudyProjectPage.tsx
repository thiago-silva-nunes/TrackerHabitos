...
import {
  ...
  CheckCircle2, Circle, Clock, FlaskConical, BookOpen,
...
export function StudyProjectPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [projectRes, tracksRes, testsRes] = await Promise.all([
        ...
        supabase.from("study_tests").select("*").eq("project_id", projectId).order("created_at", { ascending: false }),
      ]);
      ...
      if (tracksData.length > 0) {
        ...
        const tracksWithTopics: StudyTrackWithTopics[] = tracksData.map((track) => ({
          ...track,
  ...
  async function handleCreateTrack() {
    ...
    try {
      ...
      setTrackTitle("");
      const newTrack: StudyTrackWithTopics = { ...data, topics: [] };
  ...
  async function handleCreateTopic(trackId: string) {
    ...
    try {
      ...
      const { data, error } = await supabase.from("study_topics").insert({
        ...
        status: "pending",
        sort_order: track?.topics.length ?? 0,
      }).select().single();
      if (error) throw error;
  ...
  async function handleDeleteTopic(trackId: string, topicId: string) {
    try {
      ...
      setTracks((prev) =>
        prev.map((t) =>
          t.id === trackId ? { ...t, topics: t.topics.filter((tp) => tp.id !== topicId) } : t
        )
      );
    } catch {
  ...
  async function handleCreateTest() {
    if (!testForm.title.trim() || !user || !projectId) return;
    setSavingTest(true);
    try {
      const { data, error } = await supabase.from("study_tests").insert({
        project_id: projectId,
        user_id: user.id,
      ...
      navigate(`/estudos/${projectId}/testes/${data.id}`);
    } catch {
  ...
  async function handleDeleteTest(testId: string) {
    if (!confirm("Excluir este teste e todas as suas questões?")) return;
    try {
      await supabase.from("study_tests").delete().eq("id", testId);
  ...
  const donePct = allTopics.length > 0
    ? Math.round((allTopics.filter((t) => t.status === "done").length / allTopics.length) * 100)
    : 0;
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="flex gap-1 bg-surface-2 rounded-xl p-1 mb-6">
        {([["trilhas", "Trilhas de Aprendizagem", BookOpen], ["testes", "Testes", FlaskConical]] as const).map(
          ([key, label, Icon]) => (
            <button
              key={key}
              onClick={() => setTab(key)}
              className={`flex-1 flex items-center justify-center gap-2 py-2 px-3 rounded-lg text-sm font-medium transition-all ${
                tab === key ? "bg-surface-3 text-white shadow-sm" : "text-white/40 hover:text-white/70"
              }`}
            >
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
          {tracks.length === 0 ? (
            <div className="text-center py-16">
              <p className="text-white/40 text-sm mb-4">Nenhuma trilha ainda. Crie a primeira!</p>
            ...
            <>
              {tracks.map((track) => {
                ...
                return (
                  <div key={track.id} className="bg-surface-1 border border-white/6 rounded-2xl overflow-hidden">
                    {/* Track header */}
                    <button
                        ...
                        >
                          <div className="border-t border-white/6">
                            {track.topics.map((topic) => {
                              ...
                              return (
                                <div key={topic.id} className="flex items-center gap-3 px-4 py-3 border-b border-white/4 last:border-0 group hover:bg-white/[0.02] transition-colors">
                                  {/* Status toggle */}
                                  <button
                                    onClick={() => handleStatusCycle(topic, track.id)}
                                  ...
                                  <button
                                    onClick={() => navigate(`/estudos/${projectId}/topico/${topic.id}`)}
                                  ...
                                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
                                    <button
                                      onClick={() => navigate(`/estudos/${projectId}/topico/${topic.id}`)}
                                      className="p-1 rounded-lg text-white/30 hover:text-studies hover:bg-studies/10 transition-all"
                                    >
                                      <Pencil className="w-3 h-3" />
                                    </button>
                                    <button
                                      onClick={() => handleDeleteTopic(track.id, topic.id)}
                            ...
                            {addingTopicToTrack === track.id ? (
                              <div className="px-4 py-3 flex items-center gap-2">
                                <input
                                  ...
                                  className="flex-1 bg-surface-3 border border-studies/40 rounded-lg px-3 py-1.5 text-sm text-white placeholder-white/25 outline-none"
                                />
                                <Button size="sm" onClick={() => handleCreateTopic(track.id)} loading={savingTopic} disabled={!topicTitle.trim()}>
                                  Adicionar
                                </Button>
                                <button onClick={() => { setAddingTopicToTrack(null); setTopicTitle(""); }} className="p-1 text-white/30 hover:text-white">
                                  <X className="w-4 h-4" />
                                </button>
                              </div>
                            ) : (
                              <button
                                onClick={() => { setAddingTopicToTrack(track.id); setTopicTitle(""); }}
                                className="w-full flex items-center gap-2 px-4 py-2.5 text-xs text-white/30 hover:text-white/60 hover:bg-white/[0.02] transition-all"
                              >
      ...
      {tab === "testes" && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
          <div className="flex items-center justify-between mb-2">
            <p className="text-sm text-white/40">Questões de fixação para este projeto</p>
          ...
          {tests.length === 0 ? (
            <div className="text-center py-16">
              <div className="w-14 h-14 rounded-2xl bg-studies/10 flex items-center justify-center mx-auto mb-4">
                <FlaskConical className="w-7 h-7 text-studies" />
              </div>
              <p className="text-white/40 text-sm mb-4">Nenhum teste criado ainda</p>
              <Button onClick={() => setShowTestModal(true)} size="sm">
                <Plus className="w-4 h-4" /> Criar primeiro teste
              </Button>
            </div>
          ) : (
            tests.map((test) => (
              <div key={test.id} className="bg-surface-1 border border-white/6 hover:border-studies/30 rounded-2xl p-4 flex items-center gap-4 group transition-all">
                <div className="w-10 h-10 rounded-xl bg-studies/10 flex items-center justify-center flex-shrink-0">
                  <FlaskConical className="w-5 h-5 text-studies" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-white text-sm">{test.title}</p>
                  {test.description && <p className="text-xs text-white/40 truncate mt-0.5">{test.description}</p>}
                </div>
                <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all">
                  <Button size="sm" onClick={() => navigate(`/estudos/${projectId}/testes/${test.id}`)}>
                    Abrir
                  </Button>
                  <button
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-5">
                <h2 className="font-bold text-white">Nova Trilha</h2>
                <button onClick={() => setShowTrackModal(false)} className="text-white/40 hover:text-white p-1"><X className="w-4 h-4" /></button>
              </div>
              <input
                autoFocus
                value={trackTitle}
                ...
                className="w-full bg-surface-3 border border-white/8 rounded-xl px-3 py-2.5 text-sm text-white placeholder-white/25 outline-none focus:border-studies/50 transition-colors mb-4"
              />
      ...
      <AnimatePresence>
        {showTestModal && (
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-5">
                <h2 className="font-bold text-white">Novo Teste</h2>
                <button onClick={() => setShowTestModal(false)} className="text-white/40 hover:text-white p-1"><X className="w-4 h-4" /></button>
              </div>
              <div className="space-y-3 mb-5">
                <div>
                  ...
                  <input
                    ...
                    placeholder="Ex: Revisão de JavaScript básico..."
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Sobre o que é este teste?"
                    rows={2}
              ...
              <div className="flex gap-2">
                <Button variant="secondary" className="flex-1" onClick={() => setShowTestModal(false)}>Cancelar</Button>
                <Button className="flex-1" onClick={handleCreateTest} loading={savingTest} disabled={!testForm.title.trim()}>
